...
def load_and_prepare_data() -> pd.DataFrame:
    ...
    df = pd.read_csv(CLEANED_CSV, engine="python")
    print(f"[OK] Loaded {len(df)} rows from {CLEANED_CSV}\n")
...
def create_visualizations(df: pd.DataFrame):
    ...
    df["goal_interval"] = pd.cut(
        df["goal_minute_num"], bins=bins, labels=labels, right=False
    )

    plt.figure(figsize=(10, 6))
    ax = sns.histplot(
        data=df, x="goal_interval", discrete=True, kde=False, color="steelblue"
    )
    ...
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "01_goals_by_interval.png", dpi=100, bbox_inches="tight")
    plt.close()
    print("  [OK] Saved: 01_goals_by_interval.png")

    # 2. Average goal minute by season
    print("2. Average goal minute by season...")
    avg_goal_minute = (
        df.groupby("season", as_index=False)["goal_minute_num"].mean().round(1)
    )
    plt.figure(figsize=(10, 6))
    ...
    plt.tight_layout()
    plt.savefig(
        FIGURES_DIR / "02_avg_goal_minute_season.png", dpi=100, bbox_inches="tight"
    )
    plt.close()
    print("  [OK] Saved: 02_avg_goal_minute_season.png")
    ...
    goal_types.plot(
        kind="pie", autopct="%1.1f%%", colors=["lightblue", "orange", "red"]
    )
    plt.title("Distribution of Goal Types")
    plt.ylabel("")
    plt.tight_layout()
    plt.savefig(
        FIGURES_DIR / "03_goal_type_distribution.png", dpi=100, bbox_inches="tight"
    )
    plt.close()
    print("  [OK] Saved: 03_goal_type_distribution.png")
    ...
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "04_home_vs_away.png", dpi=100, bbox_inches="tight")
    plt.close()
    print("  [OK] Saved: 04_home_vs_away.png")
    ...
    plt.xticks(rotation=0)
    plt.tight_layout()
    plt.savefig(FIGURES_DIR / "05_goals_by_half.png", dpi=100, bbox_inches="tight")
    plt.close()
    print("  [OK] Saved: 05_goals_by_half.png")
...
def analyze_team_patterns(df: pd.DataFrame):
    ...
    home_avg = (
        ...
        .groupby("home_team")["goal_minute_num"]
        .mean()
        .round(1)
    ...
    away_avg = (
        ...
        .groupby("away_team")["goal_minute_num"]
        .mean()
        .round(1)
        .sort_values(ascending=False)
        .head()
    ...
    home_added = (
        df[df["team_side"] == "home"].groupby("home_team")["in_added_time"].sum()
    )
    away_added = (
        df[df["team_side"] == "away"].groupby("away_team")["in_added_time"].sum()
    )
    print(
        f"  Home teams with most added-time goals: {home_added.nlargest(3).to_dict()}"
    )
    print(
        f"  Away teams with most added-time goals: {away_added.nlargest(3).to_dict()}"
    )
