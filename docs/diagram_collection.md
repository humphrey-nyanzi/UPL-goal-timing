...
| **Active research access** | `src/research.read_sql`, the `upl_research_reader` permission template, and `cases/_case_template/analysis.ipynb` | These provide the Postgres-backed read-only case path. Issue #112 owns the detailed trust/provenance contract; the case package records the relevant state and proportionate checks. |
| **Active case package** | `cases/<case-id>-<slug>/README.md`, `analysis.ipynb`, meaningful `checks/`, `report.md`, and deliberate `outputs/` | These are the default analytical endpoint. A completed case does not automatically create an analytics object, publication, API endpoint, React route, or maintenance obligation. |
